...
function getAuctionTrips(msg, callback){
    ...
    console.log("in getAuctionTrips function");
    console.log("listId  in Handle Req:"+msg.userEmail);
    ...
    var getTrips= "SELECT * FROM auctiontrips WHERE userEmail ='"+msg.userEmail+"'";

    mysql.getData(function(err,results){


        if(err){
            console.log("Error in getting auction Trips MySQL:"+err);
        ...
        else
        {
            ...
            {
                console.log("No Auction Trips Available for user");
                res.statusCode = "401";
                callback(null, res);
