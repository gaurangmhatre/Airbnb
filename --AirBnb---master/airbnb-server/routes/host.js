...
function newListing(req ,callback)
{
  ...
  newList.safetyAmneties=req.amneties.safetyAmneties;
   ...
   newList.description=req.description.description;
    ...
    newList.auctionEndDate=req.auctionEndDate;
...
function deleteListing(req,callback){
	...
	Listing.update({"listId": req.id}, { $set: {"status": req.status} }, function(err, result){
		if(err){
			throw err;
		}else{
			if(result != ""){
...
function editListing(req,callback){
	console.log('In editListing requests queue' + req.title);
	Listing.update({"_id": req.id}, { $set: {"title": req.title, "fromDate": req.fromDate, "toDate": req.toDate, "fixedPrice": req.fixedPrice, "auctionPrice": req.auctionPrice, "guestAllowed": req.noOfGuests} }, function(err, result){
		if(err){
			throw err;
		}else{
			if(result != ""){
