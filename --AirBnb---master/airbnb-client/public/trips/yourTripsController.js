...
dashboard.controller("yourTripsController", function($scope,$http ,$state,isLoggedIn,auth) {
   ...
   /* $scope.paymentButton = [];
    ...
    $scope.viewBill= function(tripId)
    {
        ...
        console.log($scope.pendingTrips);

        for(var i=0;i<$scope.pendingTrips.length;i++)
        {
            console.log(i);
            if($scope.pendingTrips[i].tripId == tripId)
            {

                $scope.billUserName = $scope.pendingTrips[i].userName;
                console.log("Username:"+$scope.billUserName);
                $scope.tripId = $scope.pendingTrips[i].tripId;
                $scope.listingCity=$scope.pendingTrips[i].listingCity;
                $scope.listingTitle = $scope.pendingTrips[i].listingTitle;
                $scope.streetAddress = $scope.pendingTrips[i].streetAddress;
                var checkindate = new Date($scope.pendingTrips[i].checkInDate);
                $scope.checkInDate = checkindate.toISOString().split('T')[0];
                console.log("checkIn date:"+$scope.checkInDate);
                var checkoutdate = new Date($scope.pendingTrips[i].checkOutDate);
                $scope.checkOutDate = checkoutdate.toISOString().split('T')[0];
                $scope.fixedPrice = $scope.pendingTrips[i].fixedPrice;
                $scope.guestsSelected = $scope.pendingTrips[i].guestsSelected;
                $scope.cardNumber = $scope.pendingTrips[i].cardNumber.substring(12,16);
                $scope.hostEmail = $scope.pendingTrips[i].hostEmail;
