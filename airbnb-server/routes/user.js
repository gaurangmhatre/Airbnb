...
function signin(req, callback) {
    ...
    User.findOne({"local.email": req.email, "local.accountStatus": true}, function (err, user) {
        if (err) {
            return callback(err);
        } else {
            console.log(user);
...
function signup(req, callback) {
    ...
    console.log("I came till here in signup", JSON.stringify(req));
    User.findOne({
    ...
    UserProfile.findOne({"email": req.user}, function (err, result) {
        ...
        if (err) {
            ...
            return callback(null, {
                ...
                "host": result.host,
                "email": result.email,
                "profilePic": result.profileImages,
...
function getProfile(req, callback) {
    console.log("I am in the getProfile " + req.userEmail);
    UserProfile.findOne({"email": req.userEmail}, function (err, result) {
        if (err) {
            throw err;
        }
        else {
...
function editProfile(req, callback) {
    ...
    UserProfile.update({"email": req.userData.email}, {
        $set: {
            firstname: req.userData.firstName,
            lastname: req.userData.lastName,
            gender: req.userData.gender,
            birthday: req.userData.dob,
            email: req.userData.email,
            phoneNumber: req.userData.pNumber,
            preferLang: req.userData.language,
            preferCurr: req.userData.currency,
            streetAddress: req.userData.stAddress,
            city: req.userData.city,
            state: req.userData.state,
            country: req.userData.country,
            zipcode: req.userData.zipCode,
            aboutMe: req.userData.aboutme,
            workEmail: req.userData.workEmail
        }
    }, function (err, result) {
        if (err) {
            throw err;
...
function deleteHost(req, callback) {
    console.log("I am in the delete host details" + req.email);
    UserProfile.update({'email': req.email}, {$set: {'host': 'false'}}).exec(function (err, result) {
        if (err) {
            throw err;
        } else {
            console.log(result);
            if (result.length != '') {
                Listing.update({"hostId": req.email}, {$set: {"status": 'Unlisted'}}).exec(function (err, result) {
                    if (err) {
                        throw err;
                    } else {
                        callback(null, {'code': '200'});
...
function deleteAccount(req, callback) {
    console.log("I am in the delete deleteAccount " + req.email);
    User.update({'local.email': req.email}, {$set: {"local.accountStatus": false}}).exec(function (err, result) {
        if (err) {
            throw err;
        }
