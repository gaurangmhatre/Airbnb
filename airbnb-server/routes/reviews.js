...
function reviewUser(req, callback) {
    ...
    UserProfile.update({'email': req.userEmail}, {
        $push: {
            "reviewForMe": {
                ...
                "hostProfilePic": req.hostProfilePic,
                ...
                mysql.getData(function (err, res) {
                    if (err) {
                        ...
                        if (res != '') {
                            ...
                            UserProfile.update({'email': req.hostEmail}, {
                                $push: {
                                    "reviewByMe": {
                                        "feedback": req.feedback,
                                        "userReview": true,
...
function reviewList(req, callback) {
    ...
    Listing.findOne({"listId": req.listId}, function (err, result) {
        ...
        if (result != '') {
            ...
            Listing.update({"listId": req.listId}, {
                ...
                $push: {
                    "review": {
                        ...
                        "listReviewImages": req.listReviewImages
                    }
                }
            }, function (err, result) {
                if (err) {
    ...
    UserProfile.findOne({"email": req.user}, function (err, result) {
        if (err) {
            ...
            if (result != '') {
                ...
                callback(null, {
                    'code': 200,
                    'value': {'reviewByMe': result.reviewByMe, 'reviewForMe': result.reviewForMe}
                });
            } else {
                callback(null, {'code': '400'});
            }
