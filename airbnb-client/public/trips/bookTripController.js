 ...
 * http://usejsdoc.org/
 */
dashboard.controller("bookTripController", function($scope,$http ,$state,$stateParams,isLoggedIn,auth,logger)
{

    $scope.propertyClicked= function(property){
        logger.write('bookTrip',property.title,property.city);
    };
    ...
    if (typeof(Storage) !== "undefined") {
        ...
        $scope.hostProfilePic = JSON.parse(localStorage.hostProfilePic);
      //  console.log("$scope.listingDetails"+ $scope.guestsSelected);
    ...
    else
        {
            ...
            $scope.hostProfilePic=$stateParams.hostProfilePic;
        ...
        if(sguests>$scope.guestAllowed)
        {
...

            {
                ...
                if(data.statusCode==200)
                {
                    ...
                    localStorage.removeItem("hostProfilePic");
